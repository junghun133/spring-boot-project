...
public interface MemberRepository extends JpaRepository<Member, Long>{
    ...
    */
    ...
    @Query("select m from ShopMember m where m.username = :username and m.age = :age")
    List<Member> findUser(@Param("username") String username, @Param("age") int age);

    @Query("select new com.study.jpa.springdatajpa.dto.MemberDto(m.id, m.username, t.name) from ShopMember m join m.team t")
    List<MemberDto> findMemberDto();

    @Query("select m from ShopMember m where m.username in :names")
...
//            countQuery = "select count(m.username) from Member m")
    ...
    @Query("update ShopMember m set m.age = m.age +1 where m.age >= :age")
    int bulkAgePlus(@Param("age") int age);

    //////////////////////////////fetch join 여러가지 방법 -> 조인해서 한번에 연관있는 many 쪽 data를 가져오기위해
    @Query("select m from ShopMember  m left join fetch m.team")
    ...
    @EntityGraph(attributePaths = {"team"})
    @Query("select m from ShopMember m")
